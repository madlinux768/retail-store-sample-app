module "orders_service" {
  ...
  environment_variables = merge(
    {
      RETAIL_ORDERS_MESSAGING_PROVIDER   = var.azure_servicebus_enabled ? "azureservicebus" : "rabbitmq"
      RETAIL_ORDERS_PERSISTENCE_PROVIDER = "postgres"
      RETAIL_ORDERS_PERSISTENCE_NAME     = var.orders_db_name
    },
    var.azure_servicebus_enabled ? {
      RETAIL_ORDERS_MESSAGING_AZURESERVICEBUS_QUEUENAME = "orders-events"
    } : {}
  )
  ...
  secrets = merge(
    ...
    var.azure_servicebus_enabled ? {
      RETAIL_ORDERS_MESSAGING_AZURESERVICEBUS_CONNECTIONSTRING = "${aws_secretsmanager_secret_version.azure_servicebus[0].arn}:connectionString::"
    } : {}
  )
...
data "aws_iam_policy_document" "orders_db_secret" {
  statement {
    ...
    resources = concat(
      ...
      var.azure_servicebus_enabled ? [aws_secretsmanager_secret.azure_servicebus[0].arn] : []
    )
  }
}
